...
위젯을 **우클릭 → 테마** 에서 고릅니다. 선택은 저장되어 다음 실행 때 복원되고,
고르는 즉시 반영됩니다.

```bash
defaults write com.lp.turntable theme -string single45
# walnut / technics / braun / midnight / single45
```

색만 바뀌는 것이 아니라 **기계 자체가 바뀝니다**.

| 테마 | 톤암 | 판 | 데크 |
|---|---|---|---|
| 월넛 | 곧은 튜브 | 아트가 판 전체 | — |
| 테크닉스 실버 | S자 튜브 + 안티스케이팅 추 | 검은 비닐에 센터 라벨, 매트에 스트로보 도트 | 피치 페이더 · 스트로보 램프 · 시작/정지 |
| 브라운 화이트 | 아주 가는 직선 튜브 | 검은 비닐에 작은 라벨 | 로터리 노브 |
| 미드나잇 | 리니어 트래킹 레일 | 아트가 판 전체, 그루브는 굵고 성기게 | 하단 글로우 스트립 |
| 45 싱글 | 굵은 직선 튜브 | 7인치 반투명 오렌지 비닐, 도넛 홀, **45 RPM** | 로터리 노브 |

형태를 바꿔도 트래킹 기하가 깨지지 않는 데에는 이유가 있습니다.

- **S자 튜브**는 양 끝을 피벗의 수직축 위에 남겨 둔 채 가운데만 부풀립니다. 각도 계산이
  쓰는 값은 피벗–바늘 **직선거리** 하나뿐이라, 그 축을 휘게 하면 바늘이 코사인 법칙이
  가리키지 않는 곳으로 갑니다. 실물 S자 암은 헤드셸까지 오프셋시키지만 여기서는 그러지
  않습니다.
- **리니어 트래킹**은 아예 풀 각도가 없습니다. 레일이 반경 방향으로 놓이므로 재생 위치가
  각도가 아니라 거리로 바로 떨어집니다.
- **판 지름**은 테마마다 달라도 됩니다. 암 길이는 피벗 좌표에서 나오고 각도는 목표 반경에서
  역산하므로, 판을 갈아 끼우면 스윙 폭이 알아서 따라옵니다.
- **회전 속도**도 스킨이 정합니다. 45 싱글만 270°/s 로, 33⅓ 의 200°/s 보다 1.35배 빠릅니다.
  위젯을 정면으로 보고 있지 않아도 알아채는 유일한 차이입니다.

45 싱글의 도넛 홀 비율은 실물 7인치에서 그대로 가져왔습니다 — 7인치 판에 3.5인치 라벨,
1.5인치 홀. 홀을 조금만 더 키워도 라벨이 아트를 알아볼 수 없는 얇은 링이 됩니다.
반투명 비닐은 매트를 밝게 깔아야 비침이 읽힙니다. 검은 매트 위에서는 그냥 어두운 판입니다.

데크 컨트롤은 전부 **왼쪽**에 있습니다. 실물 SL-1200 은 피치 페이더를 오른쪽 아래에 두는데,
이 패널에서는 거기가 톤암이 파킹하는 자리(약 183, 143)입니다.

패널 크기만 테마에서 뺐습니다. 패널이 `TurntableView.panelSize` 로 크기를 고정하고 저장된
프레임을 이름으로 복원하는데, 테마마다 크기가 다르면 리사이즈와 프레임 복원 경로를 함께
손봐야 하기 때문입니다.

검은 비닐 스킨에서는 그루브를 **밝게, 일반 블렌드로** 그립니다. 앨범 아트 위에서는 어두운
그루브를 곱셈으로 얹는 게 맞지만, 검은 비닐 위에 검은 그루브는 그냥 사라집니다. 판 위에서
그루브를 보이게 하는 건 빛을 막는 쪽이 아니라 받는 쪽입니다.

밝은 데크(테크닉스·브라운)에서는 곡 정보 글자가 어두운 색으로 바뀝니다. 원래 텍스트가
전부 흰색으로 박혀 있어서, 팔레트를 토큰으로 뽑아야 했던 첫 번째 이유가 이것이었습니다.
암 거치대도 밝은 테마에서는 데크보다 어둡게 칠합니다 — 은색 거치대는 은색 플린스에
묻히는데, 정지 상태를 알려주는 건 결국 암이 거치대에 얹혔는지 뿐이기 때문입니다.
...
위에 멈춥니다. 그 반경은 `Theme.innermostGrooveRatio` 하나에서 나오므로 그루브를
다시 그리면 톤암의 종료 지점도 함께 따라옵니다. 센터 라벨 스킨은 라벨 바깥이 마지막
그루브라 그 값이 라벨 크기에서 파생됩니다.
...
곡 전체에 걸친 스윙은 월넛 기준 약 44도입니다. 판이 작은 스킨일수록 더 좁아집니다.
1분에 몇 도씩이라 순간적으로는 잘 보이지 않지만, 곡이 진행되면 바늘이 확실히 안쪽으로
들어옵니다.
...
아트 원본은 120×120 입니다. 월넛의 판 지름이 134pt 라 살짝 확대해 쓰고 있어서, 위젯을 더
키우면 이미지가 뭉개집니다. 센터 라벨 스킨은 아트를 판의 30~50% 로 줄여 쓰므로 원본에
여유가 생기고, 그만큼 더 또렷합니다.
...
# 스킨 고정 — 재설치 없이 테마별로 확인
TURNTABLE_THEME=braun ./Turntable.app/Contents/MacOS/Turntable

# 위치 보정량과 회전 시계를 stderr 로 출력
TURNTABLE_TRACE=1 ./Turntable.app/Contents/MacOS/Turntable 2> trace.log
```

`TURNTABLE_THEME` 은 메모리에만 얹히므로 저장된 선택을 덮어쓰지 않습니다.
그 상태에서 메뉴로 테마를 고르면 고정이 풀리고 평소대로 저장됩니다.
...
| `TonearmView.swift` | 피벗 톤암 어셈블리 (카운터웨이트·피벗·튜브·헤드셸) |
| `LinearArmView.swift` | 리니어 트래킹 암 (레일·캐리지) |
| `DeckControlsView.swift` | 플린스 위 하드웨어 (페이더·노브·램프) |
| `TurntableView.swift` | 데크 레이아웃, 톤암 각도 계산, 곡 정보 |
| `Theme.swift` | 스킨 정의 (팔레트 + 암/판 형태)와 environment 주입 |
